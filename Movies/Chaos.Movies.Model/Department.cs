...
namespace Chaos.Movies.Model
{
    ...
    public sealed class Department : Typeable<Department, DepartmentDto>
    {
        ...
        /// <inheritdoc />
        public override DepartmentDto ToContract(string languageName)
        {
            return new DepartmentDto
            {
                Id = this.Id,
                Titles = this.Titles.ToContract(languageName),
                Roles = this.Roles.ToContract(languageName)
            };
        }
        ...
        /// <exception cref="PersistentObjectRequiredException">Items of type <see cref="Persistable{T, TDto}"/> has to be saved before added.</exception>
        public override async Task<IEnumerable<Department>> ReadFromRecordsAsync(DbDataReader reader)
        {
        ...
        public override async Task<Department> NewFromRecordAsync(IDataRecord record)
        {
