...
namespace Chaos.Movies.Model
{
    ...
    public class MovieSeriesType : Typeable<MovieSeriesType, MovieSeriesTypeDto>
    {
        ...
        /// <inheritdoc />
        public override MovieSeriesTypeDto ToContract(string languageName)
        {
            return new MovieSeriesTypeDto
            {
                Id = this.Id,
        ...
        public override async Task SaveAsync(UserSession session)
        {
            ...
            {
                await service.MovieSeriesTypeSaveAsync(session.ToContract(), this.ToContract());
            }
        }
        ...
        public override async Task<IEnumerable<MovieSeriesType>> GetAsync(UserSession session, IEnumerable<int> idList)
        {
            ...
            using (var service = new ChaosMoviesServiceClient())
            {
                return (await service.MovieSeriesTypeGetAsync(session.ToContract(), idList.ToList())).Select(this.FromContract);
            }
        }
        ...
        public override async Task<IEnumerable<MovieSeriesType>> GetAllAsync(UserSession session)
        {
            ...
            using (var service = new ChaosMoviesServiceClient())
            {
                return (await service.MovieSeriesTypeGetAllAsync(session.ToContract())).Select(this.FromContract);
            }
        }
        ...
        /// <exception cref="PersistentObjectRequiredException">Items of type <see cref="Persistable{T, TDto}"/> has to be saved before added.</exception>
        public override async Task<IEnumerable<MovieSeriesType>> ReadFromRecordsAsync(DbDataReader reader)
        {
        ...
        public override async Task<MovieSeriesType> NewFromRecordAsync(IDataRecord record)
        {
