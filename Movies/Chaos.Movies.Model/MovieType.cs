...
namespace Chaos.Movies.Model
{
    ...
    public class MovieType : Typeable<MovieType, MovieTypeDto>
    {
        ...
        /// <inheritdoc />
        public override MovieTypeDto ToContract(string languageName)
        {
            return new MovieTypeDto
            {
                Id = this.Id,
        ...
        public override async Task SaveAsync(UserSession session)
        {
            ...
            {
                await service.MovieTypeSaveAsync(session.ToContract(), this.ToContract());
            }
        }
        ...
        public override async Task<IEnumerable<MovieType>> GetAsync(UserSession session, IEnumerable<int> idList)
        {
            ...
            using (var service = new ChaosMoviesServiceClient())
            {
                return (await service.MovieTypeGetAsync(session.ToContract(), idList.ToList())).Select(this.FromContract);
            }
        }
        ...
        public override async Task<IEnumerable<MovieType>> GetAllAsync(UserSession session)
        {
            ...
            using (var service = new ChaosMoviesServiceClient())
            {
                return (await service.MovieTypeGetAllAsync(session.ToContract())).Select(this.FromContract);
            }
        }
        ...
        /// <exception cref="PersistentObjectRequiredException">Items of type <see cref="Persistable{T, TDto}"/> has to be saved before added.</exception>
        public override async Task<IEnumerable<MovieType>> ReadFromRecordsAsync(DbDataReader reader)
        {
        ...
        public override async Task<MovieType> NewFromRecordAsync(IDataRecord record)
        {
