...
namespace Chaos.Movies.Model.Tests
{
    ...
    public class RatingTest
    {
        ...
        {
            var ratingSystem = GetRatingSystem(systemString);
            var rating = GetRatings(ratingString);
        ...
        private static Rating GetRatings(string ratingString)
        {
            ...
            foreach (var groupValues in ratings)
            {
                ...
                var parentRating = new Rating(int.Parse(parentValues[1]), new RatingType(uint.Parse(parentValues[0])));
                for (var i = 1; i < subRatings.Length; i++)
                {
                    var values = subRatings[i].Split(new[] {":"}, StringSplitOptions.None);
                    parentRating.AddSubRating(new Rating(int.Parse(values[1]), new RatingType(uint.Parse(values[0]))));
                }
        ...
        private static RatingSystem GetRatingSystem(string systemString)
        {
            ...
            {
                system.SetValue(new RatingType(uint.Parse(values[0])), short.Parse(values[1]));
            }
